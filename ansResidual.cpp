...
static constexpr uint32_t FILE_MAGIC   = 0x534E4152; // RANS
...
// Cap the alphabet to [0..MAX_SYM-1] and use ESC_SYM for outliers.
// Keep outliers (original int16) in a sideband vector.
struct Symbolized {
    std::vector<uint16_t> syms; // size = residuals.size(); values in [0..MAX_SYM]; ESC_SYM marks escape
    std::vector<int16_t>  esc;  // raw escape payloads in encountered order
    size_t escapes = 0;
};
                                                  ...
                                                  const std::vector<int16_t>& esc)
{
    ...
    for (size_t i=0; i<syms.size(); ++i) {
        uint16_t s = syms[i];
        if (s == ESC_SYM) out[i] = esc[ei++];
        else              out[i] = unzigzag16(s);
    }
    return out;
}
...
// ------------------------ model (static) -------------------
struct Model {
    uint32_t L = RANS_L;            // normalization
    std::vector<uint16_t> freq;     // size = ALPHABET
    std::vector<uint32_t> cdf;      // size = ALPHABET
    std::vector<uint16_t> lut_sym;  // size = L (inverse CDF)
};
...
inline Model build_model(const std::vector<uint16_t>& syms) {
    ...
    if (total == 0) {
        m.freq[0] = (uint16_t)m.L;
        total = m.L;
    }

    // Convert to probabilities  scaled to L, ensure >=1
    ...
    // Fix rounding drift to L
    while (sum > m.L) {
        // take 1 from a largest (>1) bin
...
namespace rans32 {
    ...
    static constexpr uint32_t RENORM_THR = 1u << 23;
...
struct Packed {
    ...
    int w=0, h=0, c=0;         // image shape
    uint64_t n_syms = 0;       // number of symbols
    Model model;               // freq + cdf + lut
    std::vector<uint8_t> ans_bytes; // compressed ANS payload
    std::vector<int16_t> escapes;   // escape sideband
};
...
inline void save_file(const std::string& path, const Packed& P) {
    ...
    uint32_t ALPH  = (uint32_t)P.model.freq.size();
...
struct Encoded {
    // On success:
    std::vector<uint8_t> file_bytes_estimate; // not used
    size_t escapes = 0;
    size_t n_syms  = 0;
    size_t ans_bytes = 0;
};

inline Encoded compress_residuals_to_container(const std::vector<int16_t>& residuals,
...
// Load container, decode to raw residuals (int16)
inline std::vector<int16_t> decompress_container_to_residuals(const std::string& inPath)
{
    Packed P = load_file(inPath);
    std::vector<uint16_t> syms = rans32::decode(P.ans_bytes, (size_t)P.n_syms, P.model);
    return unsymbolize_residuals(syms, P.escapes);
}

// ---------------------- helpers --------------------
inline uint64_t estimate_file_size_bytes(const Packed& P) {
    uint64_t header = 4+4+4+4+4 + 8 + 4 + 4 + (uint64_t)P.model.freq.size()*2 + 8 + 8 + 8;
    return header + (uint64_t)P.escapes.size()*2 + (uint64_t)P.ans_bytes.size();
}

inline void report_ratio(const char* tag,
                         uint64_t comp_bytes,
                         int w,int h,int c)
{
    const uint64_t pixels = (uint64_t)w*h*c;
    const double bpp = (8.0 * comp_bytes) / (double)pixels;

    const double ratio_vs_resid = (double)comp_bytes / (double)(pixels * 2ull);
    const double ratio_vs_rgb   = (double)comp_bytes / (double)(w*h*3ull);

    (void)tag; (void)bpp; (void)ratio_vs_resid; (void)ratio_vs_rgb;
}
